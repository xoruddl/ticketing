...
- **한 커밋에 파일 하나 정도만 담는다.** 엔티티 하나, 서비스 하나, 테스트 하나가 각각 커밋 하나다
  ...
  - 예외: TDD로 만든 **테스트와 그 테스트를 통과시키는 구현** (아래 "실패하는 테스트" 참고)
    - 구현은 테스트를 통과시키는 데 **꼭 필요한 파일만** 넣는다. 테스트 1개 + 구현 1개가 기본이다
    - 테스트 없이도 따로 빌드되는 파일(응답 레코드, 서비스 등)은 **먼저 따로 커밋**한다

    ```
    feat: 좌석 조회 응답 레코드를 추가한다        ← SeatResponse
    feat: 회차의 좌석을 찾는 좌석 조회 서비스를 추가한다  ← SeatQueryService
    feat: 회차 좌석 조회 API를 추가한다          ← SeatController + SeatQueryTest
    ```
