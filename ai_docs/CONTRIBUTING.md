...
- **커밋 하나를 만들면 멈춘다.** 무엇을 바꿨는지 보고하고, 사용자가 피드백을 주거나 진행하라고 할 때까지 다음 커밋을 시작하지 않는다
  - 예외: **일부러 허술하게 만드는 구간**(Step 0처럼 락·만료 처리 없이 출발점을 만드는 구간)에서는 커밋마다 멈추지 않는다.
    커밋은 그대로 작게 쪼개되, **층 하나**(예: 도메인 전부, 서비스와 API)를 이어서 커밋한 뒤 한꺼번에 보고한다.
    고쳐야 할 것을 드러내려고 만드는 코드라 한 줄씩 의논할 가치가 적고, 의논은 그 코드가 깨지는 Step 1부터가 본론이다
  - 동시성을 다루는 Step 1 이후는 다시 커밋마다 멈춘다
